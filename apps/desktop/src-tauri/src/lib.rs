...
fn journals_dir(app: &AppHandle) -> Result<PathBuf, String> {
    Ok(app_data_dir(app)?.join("journals"))
}
...
fn load_state(app: AppHandle) -> Result<AppStateData, String> {
    ...
    let metadata = fs::symlink_metadata(&path)
        .map_err(|e| format!("Could not inspect saved settings: {e}"))?;
    ...
    let state: AppStateData = serde_json::from_reader(BufReader::new(file))
...
fn execute(
    ...
    for op in &preview.operations {
        if !safe_operation_path(&root, &op.source, true)
            || !safe_operation_path(&root, &op.destination, false)
        {
...
fn list_journals(app: AppHandle) -> Result<Vec<JournalSummary>, String> {
    ...
    let entries =
        fs::read_dir(&dir).map_err(|_| "Could not read local undo history.".to_string())?;
    ...
    for item in entries {
        ...
        let Ok(journal) = sortsmith_core::journal::load_journal(&path) else {
            continue;
        };
        let available_to_undo = journal
            .entries
            .iter()
            .filter(|entry| entry.to.exists() && !entry.from.exists())
            .count();
...
fn import_state(path: String) -> Result<AppStateData, String> {
    ...
    let metadata = fs::metadata(&source)
        .map_err(|_| "Could not inspect the selected import file.".to_string())?;
    if metadata.len() > MAX_STATE_BYTES {
        return Err("The selected settings file is too large to be a SortSmith export.".into());
    }
    let file =
        File::open(&source).map_err(|_| "Could not read the selected import file.".to_string())?;
    let state: AppStateData = serde_json::from_reader(BufReader::new(file))
        .map_err(|_| "The selected file is not a valid SortSmith settings export.".to_string())?;
    validate_state_data(&state)?;
    Ok(state)
}
...
fn run_due_watches(app: AppHandle) -> Result<Vec<String>, String> {
    ...
    let preset_map = state
        .presets
        .iter()
        .map(|p| (p.id, p.rules.clone()))
        .collect::<std::collections::HashMap<_, _>>();
    let journals = journals_dir(&app)?;
    for watch in &mut state.watched_folders {
        if !watch.enabled || watch.interval_minutes < 5 {
            continue;
        }
        let due = watch.last_run_at.is_none_or(|last| {
            Utc::now() >= last + Duration::minutes(i64::from(watch.interval_minutes))
        });
        if !due {
            continue;
        }
        let Ok(root) = validated_root(&watch.path.to_string_lossy()) else {
            messages.push("A watched folder is unavailable.".into());
            continue;
        };
        let rules = watch
            .preset_id
            .and_then(|id| preset_map.get(&id).cloned())
            .unwrap_or_default();
        if rules.is_empty() {
            messages.push("A watched folder has no usable preset.".into());
            continue;
        }
        let options = ScanOptions {
            recursive: state.settings.recursive_scan,
            include_hidden: state.settings.include_hidden,
            follow_links: false,
            max_depth: Some(32),
        };
            ...
            .and_then(|p| execute_preview(&root, &p, &journals))
        {
            Ok(report) => {
                ...
                messages.push(format!(
                    "A watched folder completed {} change(s).",
                    report.completed
                ));
            }
...
fn validate_state_data(state: &AppStateData) -> Result<(), String> {
    ...
    if !matches!(state.settings.theme.as_str(), "system" | "light" | "dark") {
        return Err("Settings contain an unsupported theme value.".into());
    }
    if state.rules.len() > MAX_CUSTOM_RULES
        || state.presets.len() > MAX_PRESETS
        || state.watched_folders.len() > MAX_WATCHED_FOLDERS
        || state.recent_journal_ids.len() > MAX_RECENT_JOURNALS
    {
    ...
    for rule in &state.rules {
        sortsmith_core::rules::validate_rule(rule).map_err(|e| e.to_string())?;
    }

    let mut preset_ids = HashSet::new();
    for preset in &state.presets {
        if !preset_ids.insert(preset.id) {
            return Err("Settings contain duplicate preset identifiers.".into());
        }
        if preset.name.trim().is_empty()
            || preset.name.chars().count() > 128
            || preset.description.chars().count() > 512
            || preset.rules.len() > MAX_RULES_PER_PRESET
        {
    ...
    for watch in &state.watched_folders {
        if !watch_ids.insert(watch.id) {
            return Err("Settings contain duplicate watched-folder identifiers.".into());
        }
        let path_text = watch.path.to_string_lossy();
        if path_text.trim().is_empty()
            || path_text.chars().count() > 4_096
            || !(5..=10_080).contains(&watch.interval_minutes)
        {
        ...
        if let Some(preset_id) = watch.preset_id {
            if !preset_ids.contains(&preset_id) {
                return Err(
                    "A watched folder references a preset that is not present in this backup."
                        .into(),
                );
            }
        }
    ...
    if state
        .recent_journal_ids
        .iter()
        .any(|id| !journal_ids.insert(*id))
    {
        return Err("Settings contain duplicate undo-history identifiers.".into());
    }
    Ok(())
}
...
fn validate_unique_rule_ids(rules: &[Rule]) -> Result<(), String> {
    ...
    if rules.iter().any(|rule| !ids.insert(rule.id)) {
        return Err("Settings contain duplicate rule identifiers.".into());
    }
    Ok(())
}

fn validate_journal_paths(journal: &OperationJournal) -> Result<(), String> {
    let root = validated_root(&journal.root.to_string_lossy())
        .map_err(|_| "The journal root is unavailable or invalid.".to_string())?;
    for entry in &journal.entries {
        if !safe_operation_path(&root, &entry.to, false)
            || !safe_operation_path(&root, &entry.from, false)
        {
            return Err(
                "The undo journal contains a path outside its recorded root and was blocked."
                    .into(),
            );
        }
    }
...
fn append_operation_log(
    ...
    if !prepare_operation_log(&path, MAX_OPERATION_LOG_BYTES) {
        return;
    }
...
fn validated_root(raw: &str) -> Result<PathBuf, String> {
    ...
    if raw.trim().is_empty() || !path.exists() || !path.is_dir() {
...
fn validated_json_import_path(raw: &str) -> Result<PathBuf, String> {
    ...
    if link_metadata.file_type().is_symlink() || !link_metadata.is_file() {
        return Err(
            "The selected import must be a regular JSON file, not a link or directory.".into(),
        );
    }
    path.canonicalize()
        .map_err(|_| "Could not resolve the selected import file.".to_string())
}
...
fn validated_json_export_path(raw: &str) -> Result<PathBuf, String> {
    ...
    if path.exists() {
        let metadata = fs::symlink_metadata(&path)
            .map_err(|_| "Could not inspect the selected export path.".to_string())?;
        if metadata.file_type().is_symlink() || !metadata.is_file() {
...
fn atomic_json_write(path: &Path, value: &impl serde::Serialize) -> Result<(), String> {
    ...
    let size = writer
        .get_ref()
        .metadata()
        .map_err(|e| format!("Could not inspect temporary data: {e}"))?
        .len();
    drop(writer);
    ...
    if !path.exists() {
        return fs::rename(&temp, path).map_err(|e| {
            let _ = fs::remove_file(&temp);
            format!("Could not finalize data file: {e}")
        });
    }
...
mod tests {
    ...
    fn rotates_operation_log_at_limit() {
        ...
        assert!(!path.exists());
        assert_eq!(
            fs::read(dir.join("operations.previous.jsonl")).unwrap(),
            b"12345"
        );
