...
pub fn preview_organization(
    ...
    for item in walker.into_iter().filter_entry(|e| {
        ...
        let entry = match item {
            Ok(v) => v,
            Err(err) => {
                result.recoverable_errors.push(redact_walk_error(&err));
                continue;
            }
        };
        ...
        let file = match describe_file(root, entry.path()) {
            Ok(f) => f,
            Err(err) => {
                result.recoverable_errors.push(err.to_string());
                continue;
            }
        };
        ...
        for prepared in &prepared_rules {
            if prepared.matches(&file) {
                ...
                let destination =
                    collision_safe_path_with_reserved(&desired, &reserved_destinations);
...
pub fn execute_preview(
    ...
    for op in &preview.operations {
                ...
                Err(err) if err.kind() == std::io::ErrorKind::AlreadyExists => {
                    ...
                    if let Some(parent) = destination.parent() {
                        if let Err(parent_error) = fs::create_dir_all(parent) {
                            errors.push(format!(
                                "Could not create destination folder: {parent_error}"
                            ));
                            move_error = true;
                            break;
                        }
        ...
        if !moved && !move_error {
            errors
                .push("Could not find a collision-free destination after several retries.".into());
        }
        if moved {
            ...
            if let Err(err) = save_journal(journal_dir, &journal) {
                errors.push(format!(
                    "A completed move could not be durably recorded in the undo journal: {err}"
                ));
                break;
            }
        }
...
pub fn undo_journal(journal_path: &Path) -> Result<ExecutionReport> {
    ...
    for entry in journal.entries.iter().rev() {
        if !entry.to.exists() {
            errors.push("A moved file is missing; skipped during undo.".into());
            continue;
        }
        if entry.from.exists() {
            errors.push("Original path is occupied; skipped during undo.".into());
            continue;
        }
...
fn validate_preview_paths(root: &Path, preview: &PreviewResult) -> Result<()> {
    ...
    for operation in &preview.operations {
        let canonical_source = operation
            .source
            .canonicalize()
            .map_err(|e| io(&operation.source, e))?;
        ...
        let relative = operation
            .destination
            .strip_prefix(root)
            .map_err(|_| SortSmithError::UnsafeDestination(operation.destination.clone()))?;
        if relative.components().any(|component| {
            ...
            return Err(SortSmithError::UnsafeDestination(
...
fn validate_journal_paths(journal: &OperationJournal) -> Result<()> {
    let canonical_root = journal
        .root
        .canonicalize()
        .map_err(|e| io(&journal.root, e))?;
    for entry in &journal.entries {
        if !safe_journal_path(&canonical_root, &entry.from)
            || !safe_journal_path(&canonical_root, &entry.to)
        {
...
mod tests {
    ...
    fn previews_executes_and_undoes() {
        ...
        let preview =
            preview_organization(root.path(), &[txt_rule()], &ScanOptions::default()).unwrap();
        ...
        let journal_path = journals
            .path()
            .join(format!("{}.journal.json", report.journal.id));
    ...
    fn post_preview_destination_collision_is_never_overwritten() {
        ...
        assert_eq!(
            std::fs::read(root.path().join("Text").join("note (1).txt")).unwrap(),
            b"source"
        );
    }
    ...
    fn relative_root_execution_produces_an_undoable_absolute_journal() {
        ...
        let preview =
            preview_organization(&relative_root, &[txt_rule()], &ScanOptions::default()).unwrap();
    ...
    fn rejects_invalid_rules_before_scanning() {
        ...
        invalid.action = RuleAction::MoveTo {
            subdirectory: "../outside".into(),
        };
