...
pub fn find_duplicates(root: &Path, options: &ScanOptions) -> Result<Vec<DuplicateGroup>> {
    ...
    for item in WalkDir::new(root).follow_links(options.follow_links).max_depth(depth).into_iter() {
        let Ok(entry) = item else { continue };
        if !entry.file_type().is_file() { continue; }
        if !options.include_hidden && entry.file_name().to_str().is_some_and(|n| n.starts_with('.')) { continue; }
    ...
    let hashed: Vec<(u64, PathBuf, String)> = candidates.par_iter().flat_map_iter(|(size, files)| files.iter().filter_map(move |path| hash_file(path).ok().map(|hash| (*size, path.clone(), hash)))).collect();
    ...
    let mut groups: Vec<_> = grouped.into_iter().filter(|(_, files)| files.len() > 1).map(|((size, hash), files)| DuplicateGroup { hash, size, files: files.into_iter().map(|path| DuplicateFile { path, size }).collect() }).collect();
