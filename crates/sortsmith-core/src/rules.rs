...
mod tests {
    ...
    fn entry() -> FileEntry {
        FileEntry { path: PathBuf::from("/tmp/root/report.txt"), relative_path: PathBuf::from("report.txt"), size: 12, modified_at: None, mime: Some("text/plain".into()), extension: Some("txt".into()) }
    }

    fn rule(action: RuleAction) -> Rule {
        Rule { id: Uuid::new_v4(), name: "rename".into(), enabled: true, match_all: true, criteria: vec![RuleCriterion::Extension { values: vec!["txt".into()] }], action }
    }

    #[test]
    fn prepared_regex_matches_repeatedly() {
        let mut candidate = rule(RuleAction::MoveTo { subdirectory: "Text".into() });
        candidate.criteria = vec![RuleCriterion::NameRegex { pattern: r"^report\.txt$".into() }];
