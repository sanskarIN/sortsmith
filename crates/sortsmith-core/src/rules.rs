    ...
    pub fn new(rule: &'a Rule) -> Result<Self> {
        ...
        let regexes = rule
            ...
            .map(|criterion| match criterion {
                RuleCriterion::NameRegex { pattern } => Some(Regex::new(pattern)?),
                _ => None,
            })
            .collect();
    ...
    pub fn matches(&self, file: &FileEntry) -> bool {
        ...
        let matches = self
            ...
            .map(|(criterion, prepared_regex)| match criterion {
                RuleCriterion::Extension { values } => file.extension.as_ref().is_some_and(|ext| {
                ...
                RuleCriterion::MimePrefix { values } => file.mime.as_ref().is_some_and(|mime| {
                ...
                RuleCriterion::ModifiedOlderThanDays { days } => file
                    ...
                    .is_some_and(|modified| {
                        modified < Utc::now() - Duration::days(i64::from(*days))
                    }),
                ...
                RuleCriterion::SizeRange {
                    ...
                    min_bytes.is_none_or(|minimum| file.size >= minimum)
                        && max_bytes.is_none_or(|maximum| file.size <= maximum)
                }
        ...
        match criterion {
            ...
            RuleCriterion::NameRegex { pattern } => {
                ...
                Regex::new(pattern).map_err(|error| {
                    SortSmithError::InvalidRule(format!(
                        "invalid regex in '{}': {error}",
                        rule.name
                    ))
                })?;
            }
...
mod tests {
    ...
    fn simple_rule(action: RuleAction) -> Rule {
        rule(
            vec![RuleCriterion::Extension {
                values: vec!["txt".into()],
            }],
            action,
        )
    ...
    fn invalid_regex_is_reported_as_an_invalid_rule() {
        ...
        assert!(matches!(
            rule_matches(&candidate, &file),
            Err(SortSmithError::InvalidRule(_))
        ));
    }

    #[test]
    fn prepared_regex_matches_repeatedly() {
        let mut candidate = simple_rule(RuleAction::MoveTo {
        ...
        assert!(prepared.matches(&absolute_entry()));
        assert!(prepared.matches(&absolute_entry()));
    }
    ...
    fn rename_template_rejects_parent_escape() {
        ...
        let candidate = rule(
            ...
            RuleAction::RenameTemplate {
                template: "../{name}".into(),
            },
        );

        assert!(matches!(
            destination_for(Path::new("."), &file, &candidate),
            Err(SortSmithError::InvalidRule(_))
        ));
    }
    ...
    fn rejects_unsafe_rename_prefix() {
        let result = destination_for(
            Path::new("/tmp/root"),
            &absolute_entry(),
            &simple_rule(RuleAction::RenamePrefix {
                prefix: "../".into(),
            }),
        );
    ...
    fn rejects_unknown_template_placeholder() {
        let candidate = simple_rule(RuleAction::RenameTemplate {
    ...
    fn rename_template_preserves_extension_when_not_in_template() {
        ...
        let destination =
            destination_for(Path::new("."), &file, &candidate).unwrap();

        assert_eq!(
            destination,
            PathBuf::from("sorted-notes.txt")
        );
    }
    ...
    fn rejects_trailing_period_for_extensionless_file() {
        ...
        let mut extensionless = absolute_entry();
