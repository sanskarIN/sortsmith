...
1. **Core domain (`crates/sortsmith-core`)** — platform-neutral Rust models, rule validation/evaluation, directory scans, duplicate hashing, execution, collision handling, and undo journals. It does not depend on Tauri.
...
3. **Frontend (`apps/desktop/src`)** — React presentation and user interaction. It never performs direct filesystem operations; all privileged work crosses typed Tauri commands.
4. **Repository verification (`scripts` + `.github/workflows`)** — release metadata checks, CI quality gates, CodeQL, dependency updates, and cross-platform packaging.
